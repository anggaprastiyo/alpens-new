        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.dataSapDetail.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.data-sap-details.update", [$dataSapDetail->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('data_sap') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="data_sap_id" id="data_sap_id" required>
                                @foreach($data_saps as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('data_sap_id') ? old('data_sap_id') : $dataSapDetail->data_sap->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('jenis_program') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="jenis_program" id="jenis_program" value="{{ old('jenis_program', $dataSapDetail->jenis_program) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('item') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="item" id="item" value="{{ old('item', $dataSapDetail->item) }}" required>
                            @if($errors->has('item'))
                        ...
                        <div class="form-group {{ $errors->has('nominal') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="nominal" id="nominal" value="{{ old('nominal', $dataSapDetail->nominal) }}" step="0.01" required>
