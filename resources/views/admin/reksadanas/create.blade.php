        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.reksadana.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.reksadanas.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('tipe_asset') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="tipe_asset" id="tipe_asset" required>
                                ...
                                @foreach(App\Models\Reksadana::TIPE_ASSET_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipe_asset', 'obligasi') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
