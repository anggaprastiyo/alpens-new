            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        {{ trans('global.edit') }} {{ trans('cruds.bopDetail.title_singular') }}
                    </div>
                    <div class="panel-body">
                        <form method="POST" action="{{ route("admin.bop-details.update", [$bopDetail->id]) }}" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <div class="form-group {{ $errors->has('bop') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control select2" name="bop_id" id="bop_id" required>
                                    @foreach($bops as $id => $entry)
                                        <option value="{{ $id }}" {{ (old('bop_id') ? old('bop_id') : $bopDetail->bop->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('bop'))
                            ...
                            <div class="form-group {{ $errors->has('tahun') ? 'has-error' : '' }}">
                                ...
                                <input class="form-control" type="number" name="tahun" id="tahun" value="{{ old('tahun', $bopDetail->tahun) }}" step="1" required>
                                @if($errors->has('tahun'))
                            ...
                            <div class="form-group {{ $errors->has('nilai_bop') ? 'has-error' : '' }}">
                                ...
                                <input class="form-control" type="number" name="nilai_bop" id="nilai_bop" value="{{ old('nilai_bop', $bopDetail->nilai_bop) }}" step="0.01">
