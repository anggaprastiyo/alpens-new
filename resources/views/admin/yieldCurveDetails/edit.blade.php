                    ...
                    <div class="panel-body">
                        <form method="POST" action="{{ route("admin.yield-curve-details.update", [$yieldCurveDetail->id]) }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group {{ $errors->has('yield_curve') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control select2" name="yield_curve_id" id="yield_curve_id" required>
                                    @foreach($yield_curves as $id => $entry)
                                        <option
                                            value="{{ $id }}" {{ (old('yield_curve_id') ? old('yield_curve_id') : $yieldCurveDetail->yield_curve->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group {{ $errors->has('yesterday_yield') ? 'has-error' : '' }}">
                                ...
                                <input class="form-control" type="number" name="yesterday_yield" id="yesterday_yield" value="{{ old('yesterday_yield', $yieldCurveDetail->yesterday_yield) }}"
                                       step="0.01" required>
                            ...
                            <div class="form-group {{ $errors->has('lastweek_yield') ? 'has-error' : '' }}">
                                ...
                                <input class="form-control" type="number" name="lastweek_yield" id="lastweek_yield" value="{{ old('lastweek_yield', $yieldCurveDetail->lastweek_yield) }}" step="0.01"
                                       required>
                            ...
                            <div class="form-group {{ $errors->has('lastmonth_yield') ? 'has-error' : '' }}">
                                ...
                                <input class="form-control" type="number" name="lastmonth_yield" id="lastmonth_yield" value="{{ old('lastmonth_yield', $yieldCurveDetail->lastmonth_yield) }}"
                                       step="0.01" required>
